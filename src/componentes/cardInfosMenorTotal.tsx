...
import {  useFaturamentoTotalClienteId } from "../hooks/useFaturamento";
...
function CardInfosMenorTotal() {
  ...
  const { data: dataAbertos } = useFaturamentoTotalClienteId(
    mesAtualISO().inicioISO,
    mesAtualISO().fimISO,
    String(empresaClienteId),
    "Aberto",
  );
  const { data: dataConcluidos } = useFaturamentoTotalClienteId(
    mesAtualISO().inicioISO,
    mesAtualISO().fimISO,
    String(empresaClienteId),
    "Concluido",
      ...
      >
        ...
        >
          ...
          <strong style={{ fontSize: 14 }}>
            {Number(dataAbertos?.qntVouchers || 0) +
              Number(dataConcluidos?.qntVouchers || 0)}
          </strong>
        </p>
      </div>
      <div
        ...
        >
          ...
          >
            {Intl.NumberFormat("pt-BR", {
              style: "currency",
              currency: "BRL",
            }).format(Number(dataAbertos?.totalFaturado)) || "R$ -"}
          </p>
          <p
            style={{
              color: Cor.texto1,
          ...
          >
            {Intl.NumberFormat("pt-BR", {
              style: "currency",
              currency: "BRL",
            }).format(Number(dataConcluidos?.totalFaturado)) ||
              "R$ -"}
          </p>
        </div>
      </div>
